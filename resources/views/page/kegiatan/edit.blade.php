        ...
        <div class="col-lg-10">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="nama" class="form-control" id="nama" value="{{ old('nama') ?? $activity->nama ?? '' }}" required>
                        @error('nama')
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-select" aria-label="Default select example" name="devisi" id="devisi">
                            ...
                            <option value="{{$dv->id}}" {{ old('devisi', $activity->devisi_id) == $dv->id? 'selected' : '' }}>{{$dv->nama}}</option>
                            @endforeach
                        </select>
                        @error('devisi')
                        <div class="text-danger text-small">{{$message}}</div>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <select id="type_kegiatan" class="form-select" aria-label="Default select example" name="type_kegiatan">
                            ...
                            <option value="{{$ta->id}}" {{ old('type_kegiatan', $activity->TA_id) == $ta->id? 'selected' : '' }}>{{$ta->nama}}</option>
                            @endforeach
                        </select>
                        @error('type_kegiatan')
                        <div class="text-danger text-small">{{$message}}</div>
