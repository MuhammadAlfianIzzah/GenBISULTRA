<x-dash-layout>
    <div class="col-8">
        <a href="{{ route('create-kegiatan') }}" class="btn btn-primary"> <i class="fas fa-pen"></i> Create
            Posts</a>
                ...
                <table class="table shadow-sm rounded-3" style="overflow: hidden;">
                    ...
                    <thead class="text-white" style="background-color: #c5e3f6">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="table-responsif">Devisi</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($posts as $post)
                            <tr>
                                <th scope="row">{{ $i }}</th>
                                <td><a target="_blank"
                                        href="{{ route('detail-kegiatan', [$post->slug]) }}">{{ $post->nama }}</a>
                                </td>

                                <td class="table-responsif">{{ $post->typeKegiatan->nama }}</td>


                                <td class="table-responsif">{{ $post->devisi->nama }}</td>
                                ...
                                <td class="table-responsif">
                                    ...
                                    <img onerror="this.onerror=null;this.src='img/notfound.png';" style="width: 100px"
                                        src="{{ asset("storage/$post->thumbnail") }}" alt="{{ $post->slug }}"
                                        class="img-thumbnail">
                                </td>
                                @if (!Auth::check() || Auth::user()->hasRole('user'))
                                ...
                                @role(['admin', 'super'])
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="{{ route('izin-kegiatan', [$post->slug, 'accept']) }}"
                                                class="btn btn-success {{ $post->is_active == true ? 'disabled' : '' }}"><i
                                                    class="far fa-check-circle"></i></a>

                                            <a href="{{ route('izin-kegiatan', [$post->slug, 'null']) }}"
                                                class="btn btn-danger {{ $post->is_active == true ? '' : 'disabled' }}"><i
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('edit-kegiatan', "$post->slug") }}"
                                            class="btn btn-primary">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('delete-kegiatan', "$post->slug") }}"
                                            ...
                                            <!-- Modal -->
                                            <div class="modal fade" id="delete{{ $post->slug }}" tabindex="-1"
                                                aria-labelledby="delete{{ $post->slug }}Label" aria-hidden="true">
