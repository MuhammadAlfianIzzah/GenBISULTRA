<x-m-layout-v2>
    <div class="mt-r container py-4" style="min-height: 600px">
        <div class="row">
            <div class="col-md-12">
                <div class="h-100 p-5 text-dark shadow-sm bg-light rounded-3">
                    <h2>Aplikasi GenBI</h2>
                    <p>Aplikasi Ini dibuat oleh Anggota GenBI SULTRA, dan 100% buatan asli, kami menyadari masih ada
                        kekurangan dalam pembuatan aplikasinya oleh sebab itu mohon kritik dan sarannya, dengan kritik
                        teman-teman sekalian semoga kedepannya kami bisa membuat aplikasinya jadi lebih baik lagi</p>
                    <p class="text-danger">~ buatan GenBI SULTRA ~</p>
                </div>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-lg-4">
                <div class="card bg-light mb-3">
                    <div class="card-header text-dark text-center" style="background-color: lightblue"><i
                            class="fas fa-random"></i>
                        Aplikasi Random Picker <i class="fas fa-random"></i></div>
                    <div class="card-body">
                        Aplikasi simple untuk memilih random kandidat yang membantu kamu ketika melakukan events atau
                        pemilihan untuk mencari nama secara acak.😎
                    </div>
                    <p class="card-footer text-center">
                        <a href="{{ route('random-picker') }}" class="btn btn-outline-dark mt-2">Detail App</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="card bg-light mb-3">
                    <div class="card-header text-dark text-center" style="background-color: lightblue">
                        Aplikasi ListMiCepat <span class="badge bg-danger">New</span>
                    </div>
                    <div class="card-body">
                        Sering lupa schedule? pingin buat daftar belanja? coba deh aplikasi ini biar kamu tidak sering
                        lupa lagi!
                    </div>
                    <p class="card-footer text-center">
                        <a href="{{ route('listmi') }}" class="btn btn-outline-dark mt-2">Detail App</a>
                    </p>
                </div>

            </div>
            <div class="col-lg-4">
                <div class="card bg-light mb-3">
                    <div class="card-header text-dark text-center" style="background-color: lightblue"><i
                            class="fas fa-compress-arrows-alt"></i>
                        Aplikasi Image compress <i class="fas fa-compress-arrows-alt"></i></div>
                    <div class="card-body">
                        compress img
                    </div>
                    <p class="card-footer text-center">
                        <a href="{{ route('img-compress.show') }}" class="btn btn-outline-dark mt-2">Detail App</a>
                    </p>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="card bg-light mb-3">
                    <div class="card-header text-dark text-center" style="background-color: lightblue"><i
                            class="fas fa-qrcode"></i>
                        Aplikasi QrCode Generator <i class="fas fa-qrcode"></i></div>
                    <div class="card-body">
                        Generate Qrcode
                    </div>
                    <p class="card-footer text-center">
                        <a href="{{ route('show-qrcode') }}" class="btn btn-outline-dark mt-2">Detail App</a>
                    </p>
                </div>
            </div>
            {{-- <p>
                <a class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseExample"
                    aria-expanded="false" aria-controls="collapseExample">
                    Aplikasi Random Picker
                </a>
            </p>
            <div class="collapse show" id="collapseExample">
                <div class="card card-body">
                    <p>
                        Aplikasi ini dibuat alfianizzah , cek <a href="{{ route('random-picker') }}"
                            class="link-primary">Detail App</a>
                    </p>
                </div>
            </div>
            <p>
                <a class="btn btn-primary mt-2" type="button" data-bs-toggle="collapse"
                    data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
                    Aplikasi ListMiCepat <span class="badge bg-danger">New</span>
                </a>
            </p>
            <div class="collapse" id="collapseExample">
                <div class="card card-body">
                    <p>
                        Aplikasi ini dibuat alfianizzah , cek <a href="{{ route('listmi') }}"
                            class="link-primary">Detail App</a>
                    </p>
                </div>
            </div> --}}
        </div>
    </div>
</x-m-layout-v2>
